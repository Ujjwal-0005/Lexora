  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 pb-20 font-sans">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-dark-800 border-t-4 border-[#b48641] shadow-sm p-8 flex flex-col justify-between">
          <p className="text-xs font-bold text-gray-500 tracking-widest uppercase mb-4">PENDING REVIEWS</p>
          <h3 className="font-serif text-5xl font-bold text-[#0f172a] dark:text-white">
            {totalApplications}
          </h3>
        </div>
        <div className="bg-white dark:bg-dark-800 shadow-sm p-8 flex flex-col justify-between">
          <p className="text-xs font-bold text-gray-500 tracking-widest uppercase mb-4">TOTAL VERIFIED</p>
          <h3 className="font-serif text-5xl font-bold text-green-700 dark:text-green-500">
            {stats.total_verified_lawyers || 0}
          </h3>
        </div>
        <div className="bg-white dark:bg-dark-800 shadow-sm p-8 flex flex-col justify-between">
          <p className="text-xs font-bold text-gray-500 tracking-widest uppercase mb-4">AVERAGE UPTIME</p>
          <h3 className="font-serif text-5xl font-bold text-[#64748b]">
            99.98%
          </h3>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-dark-800 shadow-sm border border-gray-200 dark:border-dark-600">
        <div className="p-8 border-b border-gray-200 dark:border-dark-600 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <h2 className="font-serif text-2xl font-bold text-[#0f172a] dark:text-white">Active Applications</h2>
          <div className="relative w-full sm:w-auto">
            ...
            <input
              ...
              className="w-full sm:w-64 pl-10 pr-4 py-2 bg-gray-50 dark:bg-dark-700 border border-transparent focus:border-gray-300 dark:focus:border-dark-500 rounded-sm text-sm focus:outline-none transition-colors"
            />
          </div>
        </div>
        ...
        {isLoading ? (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50 dark:bg-dark-800 border-b border-gray-200 dark:border-dark-600">
                  <th className="py-4 px-8 text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest w-1/4">PROFESSIONAL</th>
                  <th className="py-4 px-8 text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest w-1/5">IDENTIFIERS</th>
                  <th className="py-4 px-8 text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest w-1/5">CREDENTIALS</th>
                  <th className="py-4 px-8 text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest w-1/6">STATUS</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-dark-600">
                {activeApplications.map((lawyer, index) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-4">
                        {lawyer.photo_url ? (
                          <img
                            ...
                            alt={lawyer.name}
                            className="w-10 h-12 rounded-sm object-cover flex-shrink-0 border border-gray-200 dark:border-dark-600"
                          />
                        ) : (
                          <div className="w-10 h-12 rounded-sm bg-[#0f172a] flex items-center justify-center flex-shrink-0">
                            <Award className="w-5 h-5 text-white" />
                          </div>
                        )}
                        <div>
                          <p className="font-serif font-bold text-[#0f172a] dark:text-white text-sm">{lawyer.name}, Esq.</p>
                          <p className="text-xs text-gray-500">{formatRoleLabel(lawyer.lawyer_profile?.designation)}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-6 px-8">
                      <p className="font-bold text-[#0f172a] dark:text-white text-xs mb-1">#RL {lawyer.id.toString().padStart(4, '0')}</p>
                      <p className="text-[11px] text-gray-500">Joined {formatDate(lawyer.created_at)}</p>
                    </td>
                    <td className="py-6 px-8">
                      <div className="flex flex-col gap-1.5 items-start">
                        <span className="bg-gray-100 dark:bg-dark-600 px-2 py-0.5 rounded-sm text-[10px] font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                    ...
                    <td className="py-6 px-8">
                      <span className="bg-[#fef3c7] text-[#92400e] px-3 py-1 rounded-sm text-[10px] font-bold uppercase tracking-wider inline-block">
                        Pending Review
                      </span>
                    </td>
                    <td className="py-6 px-8">
                      <div className="flex items-center justify-end gap-6">
                        <button
                          ...
                          className="text-[11px] font-bold text-gray-500 hover:text-[#0f172a] uppercase tracking-wider transition-colors"
                        >
                          Request Info
                        </button>
                        <button
                          ...
                          className="bg-[#166534] hover:bg-[#14532d] text-white px-5 py-2 rounded-sm text-xs font-bold uppercase tracking-wider transition-colors shadow-sm"
                        >
                          Verify
                        </button>
                      </div>
            ...
            <div className="p-6 bg-gray-50 dark:bg-dark-800 border-t border-gray-200 dark:border-dark-600 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="p-1.5 bg-white border border-gray-300 dark:border-dark-600 rounded-sm hover:bg-gray-50 dark:hover:bg-dark-700 text-gray-600 dark:text-gray-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
          ...
          <div className="text-center py-24 px-6">
            ...
            <h3 className="font-serif text-xl font-bold text-[#0f172a] dark:text-white mb-2">No Pending Verifications</h3>
            <p className="text-gray-500 font-medium text-sm">All applicants have been reviewed and processed.</p>
          </div>
        )}
      </div>

      {/* Compliance Banner */}
      <div className="relative rounded-sm shadow-2xl overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border border-amber-600/30">
        <div className="absolute inset-0 opacity-15">
          <div className="absolute top-0 right-0 w-72 h-72 bg-amber-500/20 rounded-full blur-3xl"></div>
          <div className="absolute bottom-0 left-0 w-72 h-72 bg-amber-700/10 rounded-full blur-3xl"></div>
        </div>
        <div className="relative z-10 p-10 flex flex-col gap-6">
          <div className="max-w-3xl">
            <p className="text-xs font-bold text-amber-300 uppercase tracking-[0.25em] mb-3">Compliance Standard 4.0</p>
            <h2 className="font-serif text-3xl font-bold text-white mb-3">Focused legal review, clear approval trail.</h2>
            <p className="text-amber-100/75 text-sm leading-relaxed max-w-2xl">
          ...
          <div className="grid md:grid-cols-3 gap-3">
            <div className="bg-white/5 border border-amber-600/25 rounded-sm px-4 py-3 flex items-center gap-3">
              <BadgeCheck className="w-5 h-5 text-amber-300 flex-shrink-0" />
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-amber-300/70">Designation</p>
                <p className="text-sm font-semibold text-white">Assigned by admin</p>
              </div>
            </div>
            <div className="bg-white/5 border border-amber-600/25 rounded-sm px-4 py-3 flex items-center gap-3">
              <ShieldCheck className="w-5 h-5 text-amber-300 flex-shrink-0" />
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-amber-300/70">Audit Trail</p>
                <p className="text-sm font-semibold text-white">Logged permanently</p>
              </div>
            </div>
            <div className="bg-white/5 border border-amber-600/25 rounded-sm px-4 py-3 flex items-center gap-3">
              <Building2 className="w-5 h-5 text-amber-300 flex-shrink-0" />
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-amber-300/70">Compliance</p>
                <p className="text-sm font-semibold text-white">KYC + Bar check</p>
              </div>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-t border-amber-600/20 pt-5">
            <p className="text-xs text-amber-100/60 uppercase tracking-widest font-bold">Ready for internal or regulatory review</p>
            <button
              onClick={openAuditLog}
              className="bg-amber-500 hover:bg-amber-400 text-[#0f172a] font-bold text-sm px-6 py-2.5 rounded-sm transition-colors uppercase tracking-wider shadow-md inline-block w-max"
            >
              Review Audit Log
            </button>
          </div>
        ...
        {showModal && selectedLawyer && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-dark-800 rounded-sm max-w-5xl w-full p-8 shadow-2xl max-h-[90vh] overflow-y-auto"
            >
              <div className="flex justify-between items-start mb-8 gap-4">
                <div>
                  <h2 className="font-serif text-2xl font-bold text-[#0f172a] dark:text-white">Applicant Dossier</h2>
                  <p className="text-sm text-gray-500 uppercase tracking-widest mt-1 font-bold">Confidential Review</p>
                </div>
                <button
                  onClick={() => {
                    setShowModal(false)
              ...
              <div className="mb-8">
                ...
                <textarea
                  ...
                  className="w-full p-3 border border-gray-300 dark:border-dark-600 rounded-sm bg-white dark:bg-dark-700 focus:outline-none focus:border-red-500 transition-colors resize-none text-sm"
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 border border-red-200 bg-red-50 hover:bg-red-100 text-red-700 font-bold text-xs uppercase tracking-widest py-3 rounded-sm transition-colors disabled:opacity-50"
                >
                  Deny Application
                </button>
                <button
                  ...
                  className="flex-[2] bg-[#166534] hover:bg-[#14532d] text-white font-bold text-xs uppercase tracking-widest py-3 rounded-sm transition-colors shadow-sm disabled:opacity-50"
                >
        ...
        {showAuditLogModal && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-dark-800 rounded-sm max-w-5xl w-full shadow-2xl max-h-[90vh] overflow-hidden border border-amber-600/20"
            >
              <div className="flex justify-between items-start px-8 pt-8 pb-6 border-b border-gray-100 dark:border-dark-600">
                <div>
                  <h2 className="font-serif text-2xl font-bold text-[#0f172a] dark:text-white">Recent Activity</h2>
                  <p className="text-sm text-gray-500 uppercase tracking-widest mt-1 font-bold">System Audit Log</p>
                </div>
                <button
                ...
                {isLoadingAuditLogs ? (
                  ...
                  <div className="space-y-5">
                    {auditLogs.data.map((activity) => (
                      <div key={activity.id} className="flex items-start gap-5 pb-5 border-b border-gray-100 dark:border-dark-600 last:border-b-0 last:pb-0">
